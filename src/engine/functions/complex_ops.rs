...
pub fn conj(args: &[Number]) -> Result<Number, EngineError> {
    if args.len() != 1 { return Err(EngineError::ArgumentMismatch("conj".into(), 1)); }
    match &args[0] {
...
pub fn re(args: &[Number]) -> Result<Number, EngineError> {
    if args.len() != 1 { return Err(EngineError::ArgumentMismatch("re".into(), 1)); }
    match &args[0] {
...
pub fn im(args: &[Number]) -> Result<Number, EngineError> {
    if args.len() != 1 { return Err(EngineError::ArgumentMismatch("im".into(), 1)); }
    match &args[0] {
        Number::Complex(c) => Ok(Number::Float(c.im)),
        Number::Integer(_) | Number::Rational(_) => Ok(Number::Integer(num_bigint::BigInt::from(0))),
        Number::Float(_) => Ok(Number::Float(0.0)),
    }
}

pub fn abs(args: &[Number]) -> Result<Number, EngineError> {
    if args.len() != 1 { return Err(EngineError::ArgumentMismatch("abs".into(), 1)); }
    match &args[0] {
